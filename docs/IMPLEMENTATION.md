...
`internal/context/` 提供基础的对话历史管理，上下文压缩功能将在 v0.2.0 实现。

| 文件 | 状态 | 说明 |
|------|------|------|
| `manager.go` | ✅ 基础实现 | ContextManager: AddMessage/GetMessages |
| `history.go` | ✅ 完成 | MessageHistory: 追加式消息存储 |
| `compressor.go` | 🔄 Stub | Compressor.Compress() 原样返回消息，压缩管道待 v0.2.0 |

v0.2.0 计划实现的压缩管道：
```
原始消息 → Token计数 → 语义分块 → 摘要压缩 → 关键信息提取 → 预算分配 → Output
```
...
| # | 文件/包 | 状态 | 说明 |
|---|---------|------|------|
| 10.1 | `internal/context/` | 🔄 | 基础消息管理；压缩管道 stub 待 v0.2.0 |
